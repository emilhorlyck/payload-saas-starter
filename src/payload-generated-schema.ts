...
export const repos = sqliteTable(
  ...
  (columns) => ({
    repos_tenant_idx: index('repos_tenant_idx').on(columns.tenant),
    repos_updated_at_idx: index('repos_updated_at_idx').on(columns.updatedAt),
    repos_created_at_idx: index('repos_created_at_idx').on(columns.createdAt),
  }),
)
...
export const payload_locked_documents_rels = sqliteTable(
  ...
  (columns) => ({
    ...
    payload_locked_documents_rels_repos_id_idx: index(
      'payload_locked_documents_rels_repos_id_idx',
    ).on(columns.reposID),
    parentFk: foreignKey({
    ...
    reposIdFk: foreignKey({
      columns: [columns['reposID']],
      foreignColumns: [repos.id],
      name: 'payload_locked_documents_rels_repos_fk',
    }).onDelete('cascade'),
  }),
)
...
export const relations_repos = relations(repos, ({ one }) => ({
  tenant: one(tenants, {
    fields: [repos.tenant],
    references: [tenants.id],
    relationName: 'tenant',
...
export const relations_payload_locked_documents_rels = relations(
  ...
  ({ one }) => ({
    ...
    reposID: one(repos, {
      fields: [payload_locked_documents_rels.reposID],
      references: [repos.id],
      relationName: 'repos',
    }),
  }),
...
type DatabaseSchema = {
  ...
  relations_repos: typeof relations_repos
